...
def main():
    ...
    vae = AutoencoderKLTemporalDecoder.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="vae", revision=args.revision, variant="fp16", cache_dir=args.cache_dir)
    ...
    unet, optimizer, lr_scheduler, train_dataloader, controlnext = accelerator.prepare(
        unet, optimizer, lr_scheduler, train_dataloader, controlnext
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet,controlnext):
                ...
                controlnext_image = batch["event_values"].to(
    ...
    if accelerator.is_main_process:
        ...
        if args.use_ema:
            ema_unet.copy_to(unet.parameters())
        checkpoint_dir = os.path.join(args.output_dir, 'event_motion_svd')
        pipeline = StableVideoDiffusionPipeline.from_pretrained(
            args.pretrained_model_name_or_path,
        ...
        # # Save controlnext model
        # controlnext = accelerator.unwrap_model(controlnext)
        # controlnext.save_pretrained(os.path.join(checkpoint_dir, 'controlnext'))
